...
@Test fun `SeasonDto toEntity maps id correctly`() {
    ...
    // ReviewDto
    @Test fun `should return correct author when ReviewDto is mapped`(){
        val result = createReviewDto(authorName = "Haider").toEntity()
        assertThat(result.authorName).isEqualTo("Haider")
    }


    @Test fun `should return correct id when ReviewDto is mapped`(){
        val result = createReviewDto(id = "123").toEntity()
        assertEquals(123, result.id)
    }
    @Test fun `should return correct username when ReviewDto is mapped`(){
        val result = createReviewDto(username = "haider123").toEntity()
        assertEquals("haider123", result.userHandle)
    }

    @Test fun `should return correct avatarPath when ReviewDto is mapped`(){
    ...
    @Test fun `should return correct rating when ReviewDto is mapped`(){
        val result = createReviewDto(rating = 4.5f).toEntity()
        assertEquals(4.5f, result.rating)
    }
    @Test fun `should return correct createdAt when ReviewDto is mapped`(){
    ...
    @Test fun `should handles null avatarPath when avatarPath is null`(){
        val result = createReviewDto(avatarPath = null).toEntity()
        assertNull(result.avatarUrl)
    }
    @Test fun `should handles null rating when rating is null`(){
